import { ChessColor, ChessFigure } from "./figure";
...
function moveToNotation(figure, opponentFigure, moveTo) {
    ...
    if (opponentFigure !== null) {
        return "" + figureName(figure.getFigureNumber()) + figurePosition(figure.getFigurePosition()) + "x" +
            ("" + figureName(opponentFigure === null || opponentFigure === void 0 ? void 0 : opponentFigure.getFigureNumber()) + figurePosition(moveTo.getFigurePosition())) +
            ("" + checkString);
    }
    else {
...
function setBeatenFiguresLocalStorage(opponentFigure) {
    if (opponentFigure) {
        if (opponentFigure.getFigureColorEnum() == ChessColor.White) {
            var whiteBeatenFigures = JSON.parse(localStorage.getItem("whiteBeatenFigures")) || [];
            ...
            var blackBeatenFigures = JSON.parse(localStorage.getItem("blackBeatenFigures")) || [];
...
function undoBeatenFigure(color) {
    if (color == 'black') {
        var whiteBeatenFigures = JSON.parse(localStorage.getItem("whiteBeatenFigures")) || [];
