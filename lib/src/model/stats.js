...
export function undoMove(contest) {
    ...
    if (boardFiguresByMove[0] === undefined || boardFiguresByMove[1] === undefined) {
        ...
        if (lastMove.includes('beat')) {
            if (color == 'white') {
                localStorage.setItem('beatenFigureToUndo', JSON.stringify(['black', returnFigureName(lastMove.slice(lastMove.indexOf("beat"), lastMove.length - 1))]));
            }
            else {
                localStorage.setItem('beatenFigureToUndo', JSON.stringify(['white', returnFigureName(lastMove.slice(lastMove.indexOf("beat"), lastMove.length - 1))]));
            }
        }
        else {
